...
Feature: Bank account management
  ...
  Scenario: Refuse withdrawal when funds are insufficient
    Given an account "ACC-001" belongs to "Alice" with balance 20
    When I try to withdraw 40 from account "ACC-001"
    Then the operation is refused for insufficient funds

  Scenario: Transfer money between accounts
    Given an account "ACC-001" belongs to "Alice" with balance 100
    And an account "ACC-002" belongs to "Bob" with balance 25
  ...
  Scenario: Refuse transfer when funds are insufficient
    Given an account "ACC-001" belongs to "Alice" with balance 10
    And an account "ACC-002" belongs to "Bob" with balance 25
